...
public class SimulationB{
  ...
  public SimulationB(int numberOfProcess, int burstTime, int timeSlice, int overhead, int arrivalRate, double priorityMix){
  ...
  //if the queue changes or the time slice expires, the current process is interrupted
  //if the process finishes, the timeslice is cut short.
  public void pause(Process currentProcess){
    int start = (int)System.currentTimeMillis();
    while((int)System.currentTimeMillis()-start<timeSlice && currentProcess.incomplete() && queue.peek().compareTo(currentProcess)>0){
      updateQueue();
    }
  }
